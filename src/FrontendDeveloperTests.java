...
public class FrontendDeveloperTests extends ApplicationTest {
    ...
    /**
     * This test method verifies the existence and correctness of buttons and labels in the user interface.
     * It checks if the labels have the expected text and if the buttons have the expected text.
     */
    @Test
     ...
     * It checks if the controls for the entire GUI and shortest path are created successfully.
     * It also tests the functionality of the "find" and "reset" buttons, as well as the displayed results.
     */
    @Test
    public void testCreateAllControls() {
        ...
        assertFalse(testPane.getChildren().isEmpty(), "Controls for the entire GUI and shortest path should be created.");
     ...
     * Then, it calls the methods to create shortest path controls, path list display,
     ...
     * It also performs UI interactions by clicking on buttons and asserts the expected results.
     */
    @Test
    ...
    /**
     * This method tests the creation and functionality of other controls in the GUI.
     * It creates a test pane, initializes a Frontend object, and calls methods to create
     * travel times box and optional location controls on the test pane. It then asserts
     * that the test pane is not empty, indicating that the controls have been created.
     * 
     * It also performs UI interactions by clicking on the "find" button and asserts that
     * the expected results are displayed in the path1 label. Then, it clicks on the "reset"
    ...
    /**
     * This method tests the integration of the shortest path selection functionality.
     * It creates a test pane, frontend, and backend objects, and verifies that the controls for the GUI and shortest path are created.
     * It then sets the source and destination fields, and retrieves the start and end locations.
     * Next, it finds the shortest path between the start and end locations using the backend object.
     * The method prints the shortest path for debugging purposes.
     * It then simulates clicking on the "find" button and verifies that the results list displays the expected path.
     * Finally, it simulates clicking on the "reset" button and verifies that the results list is empty.
     */
    @Test
    ...
    /**
     * This method tests the integration of getting travel times on the shortest path between two locations.
     * It verifies that the controls for the GUI and shortest path are created, and checks the results list
    ...
    public void testGetTravelTimesOnPathIntegration() {
        ...
        assertEquals("Results List (with travel times):\n\tUnion South\n\t -(182.20000000000002sec)->Atmospheric, Oceanic and Space Sciences\n\t" +
                "\n\tTotal time: 182.20000000000002sec", path2.getText());
    ...
    /**
     * Test case to verify the functionality of finding the shortest path between two locations.
     * It tests the `findShortestPath` method of the backend class by providing a start location and an end location.
     * The expected shortest path is "[Union South, Atmospheric, Oceanic and Space Sciences]".
     * It asserts that the shortest path returned by the backend is not empty and matches the expected path.
     */
    @Test
    ...
    /**
     * Test case to verify the functionality of finding the shortest path via a partner location.
     * This test case checks if the backend can correctly find the shortest path from a start location to an end location,
     * passing through a specified via location. It asserts that the returned path is not empty and prints the shortest path
     * via the via location. It also compares the actual path with the expected path using the `assertEquals` method from the
     * `Assertions` class.
     * 
